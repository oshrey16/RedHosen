...
class InProgressReports extends StatefulWidget {
  const InProgressReports({Key? key}) : super(key: key);

  @override
  State<InProgressReports> createState() => _ActiveReportsState();
}

class _ActiveReportsState extends State<InProgressReports> {
  ...
  void initState() {
    super.initState();
    allReports = AllActiveReports.create(n: 1);
                    ...
                    if (snapshot.hasError) {
                      ...
                      if (snapshot.data != null) {
                        if (snapshot.data!.test.entries.isEmpty) {
                          return const Center(
                              child: Text('לא קיימים דיווחים חדשים'));
                        }
                        return ListView(
                            shrinkWrap: true,
                      ...
                      Visibility(
                        child: Row(
                          children: [
                            ...
                            ElevatedButton(
                              onPressed: () {
                                navigateTo(_selectedPoints.latitude,
                                    _selectedPoints.longitude);
                              },
